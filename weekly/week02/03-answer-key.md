# Week 2 答案與詳解：How Minecraft Was Created

## Part 2：Reading Comprehension

1. **B** - 文章主線是 Notch 公開早期版本、聽取玩家意見並持續改進 Minecraft。
2. **C** - 第三段指出最初名稱是 “Cave Game.”
3. **A** - 第四段說他把早期版本分享到網路上。
4. **B** - 玩家測試後寫下意見並提出想法。
5. **A** - 遊戲常有改變，玩家想回來看看有什麼新內容。
6. **A** - 前一句的「告訴 Notch 喜歡什麼、哪裡有問題」就是 `feedback`。
7. **C** - 結尾強調玩家回饋幫助 Notch 持續改進作品。

## Part 4：Grammar Practice

### A. Choose the best question word

1. **Who** - 答案是人 Notch。
2. **When** - 答案是時間 In May 2009。
3. **Where** - 答案是地點 Online。
4. **What** - 答案是做了什麼事。
5. **Why** - 答案說明目的／原因。
6. **How** - 答案說明改進的方法。

### B. Put the words in order

1. **Where did Notch share the game?**
2. **Why did players come back?**
3. **What was the first game like?**
4. **Who created Minecraft?**

### C. Find and fix the mistake

1. **How did Minecraft become popular?** - `did` 後用原形 `become`。
2. **Who created the first game?** - `Who` 是動作者時直接接動詞。
3. **When did Notch start the project?** - `did` 放在主詞 Notch 前面。

## Part 5：Speaking

開放作答。檢查每題的疑問詞是否符合答案類型，並留意：

- `Who + 動詞 ...?`
- `疑問詞 + be + 主詞 ...?`
- `疑問詞 + do/does/did + 主詞 + 原形動詞 ...?`

## Homework：Vocabulary Review Quiz

### A. Matching

1. **C** - project：專案；計畫
2. **D** - share：分享；公開給他人
3. **A** - improve：改善；改進
4. **B** - company：公司

### B. Fill in the blank

5. **test**
6. **simple**
7. **player**
8. **feedback**
9. **program**

### C. Multiple choice

10. **A** - suggest 是提出或給予想法。
11. **A** - popular 指受到許多人喜愛或注意。
12. **B** - continue 指繼續進行。

### D. Translate

13. **創造／製作一個新世界**
14. **player feedback / players' feedback**

> Homework 涵蓋全部 13 個核心字：project, share, improve, company, test, simple, player, feedback, program, suggest, popular, continue, create。
