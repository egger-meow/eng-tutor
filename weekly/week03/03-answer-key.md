# Week 3 答案與詳解：The Town Notice and the Guardian of the Dojo
...
1. **some** - `medicine` 為不可數名詞，不可用 a/an，此處為肯定句用 some。
2. **many** - `food stalls`（攤位）為可數複數名詞，用 many 修飾。
3. **a few** - `friends`（朋友）為可數複數名詞，表示「有一些朋友」用 a few。
4. **an ; the** - 第一格 `exciting` 為母音發音開頭用 an；第二格再次提到特定那場表演用 the。
5. **any** - 疑問句中修飾不可數名詞 `water` 用 any。
6. **a lot of** - `students` 為可數複數，不可用 much 或 an；a lot of 可修飾可數複數。
...
## Part 5：Speaking & Discussion

開放作答。教學重點在於口語造句時能正確使用：
- `There is + 不可數 / 單數名詞...` (例如：*There is clean water in the well.* / *There is a stranger outside.*)
- `There are + 複數名詞...` (例如：*There are a few guards on the bridge.* / *There are three patients in the clinic.*)
...
1. **D** - protect：保護；防衛
2. **E** - patient：病人；有耐心的
3. **A** - medicine：藥品；藥物 (不可數)
4. **B** - peaceful：和平的；平靜的
5. **C** - recover：康復；恢復健康
...
6. **event** - 煙火大會是最令人興奮的夏日活動。
7. **guard** - 石像佇立在門外以守衛古寺。
8. **stranger** - 警察叮嚀孩子不要拿陌生人的食物。
9. **strength** - 健康飲食與規律運動能給予身體更多力氣。
10. **crowd** - 熱情的人群在表演開始時大聲歡呼。
11. **notice** - 進入泳池前請閱讀牆上的警告公告。

---

### C. Contextual Multiple Choice

12. **A** - promise 意為許下承諾／給予保證。
13. **B** - martial arts skill 意為武術技巧。
14. **A** - fireworks 意為煙火。
...
15. **Hakuji promised to protect Koyuki.**（狛治承諾要保護戀雪。）
16. **There is a lot of clean water in the well. (或 There is much clean water in the well.)**（水井裡有許多乾淨的水。）

> 💡 Homework 完整涵蓋本週全部 14 個核心單字：protect, patient, medicine, peaceful, recover, event, guard, stranger, strength, crowd, notice, promise, skill, firework。
